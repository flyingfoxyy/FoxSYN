...
建议为命令增加 `-T num`，范围 1–3，默认 3。`-T 1` 只运行 legacy-compatible 轨迹，用于快速回归和低运行时间场景。

每条轨迹从 CSR 入口网络的独立 `Abc_NtkDup` 快照开始。ABC 网络复制保留对象 `part_id` 和网络 `pPdb`，轨迹之间不共享可变网络状态。
...
2. **精确验证**：对可逆操作暂时应用，再核对真实指标；对 MFS resub 则在提交前完成尽可能精确的预测，并在轮末执行全网审计。
...
若审计失败，恢复轮初快照并停止当前轨迹。轨迹失败不影响其他轨迹；全部轨迹失败时恢复 CSR 入口网络并返回失败。
...
一个序列可以包含暂时不降低 cut-edge 的中间步骤，但中间状态仍不得违反 hop、cut-net 和分区平衡硬约束。整个序列只有在最终 cut-edge 严格下降时才原子提交。

Phase 0 只修改 `part_id`，不创建逻辑节点，因此不消耗节点增长额度。

### 4.3 轨迹差异

- legacy-compatible：保持当前候选优先级和稳定 tie-break；
...
保留最多 64 个高价值 divisor。双 divisor 只枚举 counterexample 覆盖互补的组合，每个节点最多尝试 256 个组合，不做完整平方级排列。

### 5.4 多成功方案选择

Phase 1 不在第一个 SAT 成功方案出现时立即更新网络，而是每个节点最多收集 8 个 SAT 成功方案，按以下顺序选取：
...
提交前从新 fanin 集合预测 cut-edge、受影响 cut-net、hop arrival 和 Shannon 分解节点增量。Phase 1 每轮保存网络快照；若提交后的轮末审计发现预测不一致，恢复整轮、拉黑相关候选，并继续下一轮。
...
1. 暂时接入复制簇并改接目标 fanout；
2. 从受影响 fanout 开始向 TFO 传播 hop-arrival；
3. 记录所有被修改的 arrival，形成回滚日志；
4. 任一节点超过 `hop_limit` 时，恢复拓扑和 arrival；
5. 接受后保留更新后的 HopState。

每轮结束全网重算 hop，校验增量传播结果。不一致时恢复轮初网络。
...
- MFS resub：依靠轮初网络快照处理无法安全逆向恢复的更新。
...
```text
-T num  : deterministic search trajectories, 1-3 [default = 3]
```

现有 `-G` 节点增长百分比继续保留，但其预算改为相对于 CSR 入口节点数，并由 Phase 1/2 共享。预算按累计正净增长单调消耗，删除节点不返还额度。默认仍为 2%。本设计确认的稳健配置使用 `-G 2`。
...
- 几何平均运行时间不超过当前 CSR 的 5 倍；
- 单个大型案例允许最多 8 倍，但必须有超时限制；
...
1. 统一 `OptimizationState`、指标口径和确定性排序；
2. legacy-compatible 单轨迹回归，确保基础行为可比较；
