...
## 12.5 Phase 0 实测结果（v1 分组：单 LUT / 小连通分量）

`scripts/run_csr4_measure.py` 跑了一遍 mcnc 全套（`if -K 6; hpart -N 2`，默认 `-B 12 -L 8`）：29 个 case 全部 OK，0 失败、0 超时。

**聚合：60 / 2470 crossing = 2.4%**（detected-floor，只算组合层 ODC；分母是去重后的物理跨界线数，即 `ps` 打印的 `cut-edge`）。

Top 案例：

| case | recoverable | crossing | pct |
|---|---|---|---|
| mcnc_C880 | 3 | 20 | 15.0% |
| mcnc_C7552 | 2 | 17 | 11.8% |
| mcnc_apex4_2 | 14 | 153 | 9.2% |
| mcnc_k2 | 6 | 87 | 6.9% |
| mcnc_spla | 13 | 287 | 4.5% |

其余 24 个 case 都在 0%~3.7% 之间，其中 12 个报 0.0%（组合层仪器探不到，读作 detected-floor 而非 zero，见 §5/§9 的口径声明）。

**这比 csr3 Phase 0 在同一套 mcnc 上测到的聚合结果（749/2059 = 36.4%，见 `docs/csr3-phase0-plan.md` 交付记录）低一个量级。** 两者测的不是同一种水分（csr3 测 A 侧可达性 SDC，csr4 测 B 侧可区分性 ODC），所以不能直接相减，但差距之大值得如实记录，不夸大 csr4 v1 的收益：

- v1 的分组（相同 bound-set 或小连通分量，`-B 12 -L 8` 截断）大概率把大部分"跨组共享"的机会漏掉了——`docs/csr4.md` §13 已经点出这是后续方向，这份数据是它排在优先级第一位的直接证据。
- all-sinks 条件（§5/§6）本身会过滤掉不少候选：任何一根跨界线只要在 B 侧还有一个不在本组内的 sink，就整根降级进 `B_keep`，不参与压缩。
- 只测了当前 cut 边界；mapper 选择不同 cut 会暴露更多/更少的跨界叶子，真实可得水分的上下界还未知。

**结论**：数据本身不否定 csr4 的机制（§4 标杆例子和单测证明机制是对的），但否定了"v1 的保守分组能挖到可观收益"这个假设。下一步该往哪走——改进分组算法，还是先接受"这条路的净收益本来就小于 csr3"——是需要基于这份数据做的决定，不在本次 Phase 0 交付范围内。
