...
**这条判据对 `v` 的依赖只经由 `vw[v]`**。记 `slack[T] = max_weight - wsum[T]`，则「`v` 可移入 `T`」等价于 `vw[v] <= slack[T]`。由此有两条实现上的推论：

- 顶点权重全为 1 时，可行性只取决于 `T`，与选哪个 `v` 无关。§5.3 的桶内向下扫描要么第一个就命中，要么整侧都不可行。
- 一般情况下，先用全局 `min_vw = min_v vw[v]` 做一次早退检查：`slack[T] < min_vw` 时整侧必然不可行，无需扫描。

### 5.2.1 复杂度限定

§5.4 末尾的「一趟 pass 为 O(pins)」在**顶点权重均匀**时成立（含全为 1 的默认情形）——此时按上面第一条推论，桶内扫描不会真的向下走。

顶点权重差异很大、且状态贴近平衡边界时，§5.3 的桶内扫描每次移动最坏是 O(n)，一趟退化到 O(n²)。这是「桶顶最优」与「O(1) 选取」之间的固有张力，本设计选择保证解质量。上面的 `min_vw` 早退能挡掉「扫遍全侧却一无所获」这一最常见的退化路径。

本次交付的两个实例化（`SimpleHypergraph` 测试用例、`AbcNtkWrapper`）顶点权重都是 1，落在 O(pins) 分支上。此限定在此记录，供将来引入非均匀权重时参考。
...
一趟 pass 的总代价是 O(pins)，这是 FM 的全部价值所在。适用条件见 §5.2.1。
