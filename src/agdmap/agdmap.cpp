...
//===----------------------------------------------------------------------===//
void
Node::cutEnum(int lut_size, bool area_oriented) {
    ...
    if (area_oriented) {
        ...
        for (int i = 1; i != cuts_.size(); ++i) {
            pCut cut = cuts_[i];
            if (min_dep_area_->depth() > cut->depth() ||
                (min_dep_area_->depth() == cut->depth() && min_dep_area_->getArea() > cut->getArea())) {
                min_dep_area_ = cut;
            }
        }
    }
...
void
Node::setAig(Node *fanin0, Node *fanin1, bool comple0, bool comple1) {
    fanins_[0] = fanin0;
    fanins_[1] = fanin1;
    comple_[0] = comple0;
    comple_[1] = comple1;
...
inline Depth
Node::depth() {
    return min_dep_area_ == nullptr ? 0 : min_dep_area_->depth();
}

pCut
Node::mergeCut(const pCut &left, const pCut &right, int lut_size, bool area_oriented) {
...
inline pCut
Node::trivCutGen(bool area_oriented) {
...
NodeSol
Node::lutSol(Solution *sol, bool if_div, bool flow, Level required) {
    if (cuts_.size() == 1) {
        assert(virtual_);
...
NodeSol
Node::minAreaIncSol(Solution *sol, bool flow, Level required) {
    bool if_div = true;
...
NodeSol &
NodeSol::operator+=(const NodeSol &nSol) {
    if (this == &nSol)
        return *this;
    this->area += nSol.area;
...
Cut::Cut(const pGate &gate) {
    inputs_.insert(inputs_.end(), gate->inputs().begin(), gate->inputs().end());
}

Area
Cut::localArea(bool if_div) {
    Area area = 0;
...
void
Solution::add(Node *node, const pCut &rep) {
    sol_[node] = rep;
    if (rep == nullptr) {
        weight_[node]++;
    } else {
...
float
getCutExactArea(const pCut &cut, std::map<Node *, float> &exact) {
    float area = 0;
...
Mapper::init() {
    ...
    for (int i = 0; i < pNtk_->vObjs->nSize; ++i) {
        ...
        Node *node = nullptr;
        if (abc_obj->Type == 2) {
            node = new Node(Node::Pi, abc_obj->Id, Abc_ObjName(abc_obj), Abc_ObjFanoutNum(abc_obj), abc_obj->fPhase,
                            cuts_num);
            Pi_.push_back(node);
            ...
            node = new Node(Node::And, abc_obj->Id, Abc_ObjName(abc_obj), Abc_ObjFanoutNum(abc_obj), abc_obj->fPhase,
                            cuts_num);
            nodes_.push_back(node);
        } else {
            // const 1
            node = new Node(Node::Cns, abc_obj->Id, Abc_ObjName(abc_obj), Abc_ObjFanoutNum(abc_obj), abc_obj->fPhase,
                            cuts_num);
            const_1_ = node;
        }
        ...
        for (Node *node : nodes_) {
            if (node->choice.size() != 0) {
                for (Node *ch_node : node->choice) {
                    ...
                    ch_node->setChoice();
                    ch_node->setFanoutNum(node->fanoutNum());  // fanout num of choice nodes(exclude
                                                               // chice root) shall be 0
                }
...
Mapper::map() {
    ...
    // compare
    if (lut_sol->getArea() < area_sol->getArea() ||
        (lut_sol->getArea() == area_sol->getArea() && lut_sol->getLevel() < area_sol->getLevel())) {
        delete area_sol;
    } else {
        delete lut_sol;
        lut_sol = area_sol;
    ...
    double total_elapse_time =
        (std::chrono::duration<double, std::milli>(elapse_end - elapse_start)).count() / 1000.0;  // ms->sec

    if (verbose()) {
        int enumerate = (std::chrono::duration<double, std::milli>(elapse_enum_end - elapse_start)).count() / 1000.0 /
                        total_elapse_time * 100.0;
        int select = (std::chrono::duration<double, std::milli>(elapse_sel_end - elapse_enum_end)).count() / 1000.0 /
                     total_elapse_time * 100.0;
        std::string space = " ";
        std::string empty = "";
...
void
Mapper::cutSel(Solution *sol, bool area_oriented, bool flow) {
...
void
Mapper::updateFlow(Solution *sol) {
    for (auto it = begin(); it != end(); ++it) {
        Node *node = *it;
...
void
Mapper::itrSel(Solution *&initial_sol, int max_itr_num, bool area_oriented, bool flow) {
    int num = 1;
    while (max_itr_num != 0) {
        ...
        if (new_area < init_area ||
            (area_oriented == false && new_area == init_area && new_sol->getLevel() < initial_sol->getLevel())) {
            delete initial_sol;
            initial_sol = new_sol;
...
void
Mapper::simpleGateGen(int gate_size_upper) {
...
std::string
Mapper::expToTrueValue(const std::vector<Node *> &inputs, const std::string &exp) {
...
//===----------------------------------------------------------------------===//
bool
SimpleGate::eat(Node *node, int idx) {
    bool val = false;
...
void
SimpleGate::expand(int gate_size) {
...
void
SimpleGate::SimpleGateEnum(int k, bool area_oriented, std::vector<Node *> &nodes) {
    ...
    if (area_oriented) {
        comparator = [](Node *lhs, Node *rhs) { return lhs->area() > rhs->area(); };
    } else {
    ...
    if (area_oriented == false) {
        for (int i = 1; i != root_->cutListNum(); ++i) {
            if (rep_cut->depth() > root_->getCut(i)->depth() ||
                (rep_cut->depth() == root_->getCut(i)->depth() && rep_cut->getArea() > root_->getCut(i)->getArea())) {
...
void
SimpleGate::combine(int k, bool area_oriented) {
    ...
    if (area_oriented) {
        ...
        while (combine_num != size()) {
            ...
            pruner.reset_cell_size_upper(max_dec_size,
                                         combine_num == size() - 1 ? max_dec_store_num : each_size_dec_num);
...
void
Decompose::multilevel_decompose() {
    int k = lut_size();
...
pCut
Decompose::kcut(Node *root, const std::set<Node *> &complemented, bool area_oriented) {
    if (area_oriented) {
...
void
Decompose::cutEnum(std::vector<pCut> &cuts, const std::set<Node *> &complement, int lut_size, bool area_oriented) {
...
//===----------------------------------------------------------------------===//
pCut
Bin::cutGen(const std::set<Node *> &complemented, std::vector<Node *> &virtual_nodes) {
    if (cut_) {
        return cut_;
    }
...
Node *
Bin::cutEnum(std::vector<Node *> &choice_nodes, const std::set<Node *> &complement, int lut_size, bool area_oriented) {
...
Node *
Bin::cutsMerge(Node *fanin1, Node *fanin2, const std::set<Node *> &complement, int lut_size, bool area_oriented) {
