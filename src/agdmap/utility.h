...
public:
    struct Implicant {
        ...
        int ones;
        bool used;
        Implicant(int var_cnt, int i = 0, std::vector<int> min = std::vector<int>(), std::string t = "", int m = 0,
                  bool u = false)
        ...
        bool
        operator<(const Implicant &b) const {
            return ones < b.ones;
        }
        std::vector<int>
        cat(const Implicant &b) {
...
public:
    ...
    void
    addVar(const std::string &var_name) {
        vars_.push_back(var_name);
    }
    void
    set_output(const std::string &name) {
        output_ = name;
    }
    ...
    std::string
    getMinExpression(TYPE true_value) {
        const float b = 2.0f;
        ...
        while (implicants.size() > 1) {
            ...
            for (size_t i = 0; i < implicants.size() - 1; ++i)
                for (size_t j = i + 1; j < implicants.size(); ++j)
                    if (implicants[j].ones == implicants[i].ones + 1 && implicants[j].mask == implicants[i].mask &&
                        count1s(implicants[i].implicant ^ implicants[j].implicant) == 1) {
                        implicants[i].used = true;
                        implicants[j].used = true;
                        aux.push_back(
                            Implicant(var_cnt(), implicants[i].implicant, implicants[i].cat(implicants[j]),
        ...
        std::vector<std::vector<bool>> table(primes.size(), std::vector<bool>(minterms.size()));
    ...
    TYPE
    getTruthValue(const std::string &string) {
        ...
        for (int i = 0; i < value_cnt; i++) {
            ...
            for (size_t j = 0; j < elements.size(); j++) {
                if (operators[j] == 0) {  // vars
                    stack.push(values[elements[j]]);
                } else if (operators[j] == 1) {  // const 0, 1
...
protected:
    ...
    void
    mul(std::vector<uint64_t> &a, const std::vector<uint64_t> &b) {
    ...
    void
    infix2Suffix(const std::string &infix, std::string &suffix) {
        suffix.clear();
...
private:
    ...
    /**
     *  the ordered cells, index by cell size. In each list the cell will be unique, which means
     * list will store only one cell if two or more cells are equivalent.
     */
...
public:
     ...
     */
    void
    collect(std::vector<Cell> &cells, float comp_epsilon) {
     ...
     */
    void
    collect(std::vector<Cell> &cells, bool area_oriented) {
        ...
        for (size_t i = 1; i != __cells.size(); ++i) {
            ...
            for (auto it = cell_set.rbegin(); it != cell_set.rend(); ++it) {
                ...
                {
                    cells.emplace_back(*it);
                } else if (__valuer(*it) <= value_upper &&
                           (cells.size() == 0 || __valuer(*it) < __valuer(cells.back()))) {
                    cells.emplace_back(*it);
                }
            }
        }
     ...
     */
    void
    reset_store_num_upper(const std::vector<short> &new_max_store_num) {
    ...
    /**
     * @brief reset the max cell size to new_cell_size_upper and set the max store number by
     * max_store_num(default = 4)
     *
     * @param new_cell_size_upper
     */
    void
    reset_cell_size_upper(int new_cell_size_upper, int new_store_num = 0) {
        __cells.clear();
...
void
ordered_merge(const std::vector<Value> &vec_a, const std::vector<Value> &vec_b, std::vector<Value> &result) {
    size_t idx_a = 0;
    size_t idx_b = 0;
    _less less;
...
public:
     ...
     */
    truth_table &
    operator=(const std::string &expresstion) {
    ...
    void
    swap(truth_table &tt) {
        expression_.swap(tt.expression_);
    }
...
//===----------------------------------------------------------------------===//
static std::string
int2Hex(unsigned long val, int len) {
    std::string init;
    init.resize(len);
    for (int i = 0; i < len; ++i) {
        int cc = val & 15UL;
