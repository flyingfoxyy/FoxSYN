...
public:
    ...
    bool
    isAreaOriented() const {
        return area_oriented_;
    }
    bool
    verbose() const {
        return verbose_;
...
public:
    ...
    Node(NodeType type, int id, const std::string& name, int fanout, bool phase, int cuts_num)
        : type_(type),
          fanout_num_(fanout),
          id_(id),
          name_(name),
          area_(0),
          flow_(0),
          min_dep_area_(nullptr),
          simple_gate_(nullptr),
          mark_(-1),
          virtual_(false),
          phase_(phase),
          choice_root_(false) {
...
public:
    ...
    pCut
    minDepth() const {
        return min_dep_area_ == nullptr ? minArea() : min_dep_area_;
    }
    pCut
    minArea() const {
        return cuts_.front();
    ...
    void
    setSgate(const pGate& gate) {
        simple_gate_ = gate;
    }

    void
    addCut(const pCut& cut) {
...
public:
    ...
    Cut(Node* node) {
        inputs_.emplace_back(node->fanin(0));
        inputs_.emplace_back(node->fanin(1));
    }
...
struct comp_pcut_area {
    bool
    operator()(const pCut& lhs, const pCut& rhs) const {
...
static bool
comp_ppcut_size_greater_func(const Cut* lhs, const Cut* rhs) {
...
static bool
comp_ppcut_delay_func(const Cut* lhs, const Cut* rhs) {
    if (lhs->depth() != rhs->depth()) {
        return lhs->depth() < rhs->depth();
    }
    if (lhs->cutsize() != rhs->cutsize()) {
...
private:
    ...
    bool
    isAreaOriented() const {
        return para_.isAreaOriented();
    }
    int
    getLutSize() const {
        return para_.getLutSize();
    }
    int
    getGateSize() const {
        return para_.getGateSize();
    }
    bool
    verbose() const {
        return para_.verbose();
    }
};
...
public:
    ...
    bool
    isRoot(Node* node) {
        return sol_.find(node) != sol_.end();
    }
    pCut
    repCut(Node* node) {
        assert(isRoot(node));
    ...
    bool
    shared(Node* node) {
        return shared_prohibited_.find(node) == shared_prohibited_.end();
    }

    int
    fanout(Node* node) {
        return weight_.find(node) == weight_.end() ? 1 : weight_[node];
    }
    int
    reference(Node* node) {
        return weight_.find(node) == weight_.end() ? 0 : weight_[node];
    }
    void
    setRef(Node* node) {
        assert(weight_.find(node) == weight_.end());
        weight_[node] = 1;
    }
...
public:
    ...
    Bin(Cut* cut) : root_(nullptr), sign_(cut->sign()), depth_(0), inputs_(cut->inputs()) {
        cuts_.reserve(3);
        cuts_.push_back(cut);
    }

    Bin() : root_(nullptr), sign_(0), depth_(0) {}

    Bin(const Bin& bin) = default;

    Bin(Bin* bin) : root_(nullptr), sign_(bin->sign_), depth_(bin->depth_), inputs_(bin->inputs_), cuts_(bin->cuts_) {
...
public:
    ...
    int
    cutsize() const {
        return inputs_.size() + connect_.size();
    }
    ...
    Node* cutEnum(std::vector<Node*>& choice_nodes, const std::set<Node*>& complement, int lut_size,
                  bool area_oriented);
...
struct comp_pbin_size {
    bool
    operator()(const Bin* lhs, const Bin* rhs) const {
        return lhs->size() > rhs->size();
    }
};

static bool
comp_pbin_size_less(const Bin* lhs, const Bin* rhs) {
    return lhs->size() < rhs->size();
}
...
public:
    ...
    Decompose(Decompose* decomp, std::vector<Bin*>& bins)
        : cut_size_(0), lut_size_(decomp->lut_size_), area_(decomp->area_), depth_(decomp->depth_), root_bin_(nullptr) {
        bins_.swap(bins);
    }
    ...
    Decompose(std::vector<Cut*>& cuts, int k) : cut_size_(0), lut_size_(k), area_(0), depth_(0), root_bin_(nullptr) {
     ...
     */
    pCut
    bins2cuts(const std::set<Node*>& complemented) {
...
struct comp_pdec_delay {
    bool
    operator()(const pDec& lhs, const pDec& rhs) const {
        if (lhs->depth() == rhs->depth()) {
...
static bool
comp_pdec_area_fn(const pDec& lhs, const pDec& rhs) {
    return lhs->area() < rhs->area();
}

struct sizer_pdec {
    int
    operator()(const pDec& pdec) const {
        return pdec->input_size();
    }
};
...
static int
countOnes64(unsigned long uWord) {
