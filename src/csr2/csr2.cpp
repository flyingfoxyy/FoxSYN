                             ...
                             int &total_attempts, int &total_successes)
{
    ...
    for (int round = 0; round < cfg.max_rounds; ++round)
    {
        ...
        if (state.growth.remaining() < growth || !state.Audit())
        {
            ...
            state.AttachNetwork(pNtk);
            if (cfg.verbose)
                printf("csr2: phase1 round %2d rolled back by hard constraints\n", round);
            break;
        }
        ...
        if (cfg.verbose)
            printf("csr2: phase1 round %2d  candidates=%3zu  fixed=%3d  cut-edges=%d\n",
                                 ...
                                 int &total_replications)
{
    ...
    for (int round = 0; round < cfg.max_rounds; ++round)
    {
        ...
        {
            if (cfg.verbose)
                printf("csr2: phase2 node budget exhausted (%d/%d), stopping\n",
        ...
        {
            if (cfg.verbose)
                printf("csr2: phase2 incremental hop verification failed\n");
            break;
        }

        if (cfg.verbose)
            printf("csr2: phase2 round %2d  candidates=%3zu  replicated=%3d"
                            ...
                            detail::OptimizationState &state, int &total_swaps)
{
    ...
    for (int round = 0; round < cfg.max_rounds; ++round)
    {
        ...
        if (cfg.verbose)
        {
            ...
            printf("csr2: phase0 swap round %2d  swaps=%3d  cut-edges=%d  hop=%d/%d\n",
                                ...
                                int &total_compound)
{
    ...
    for (int round = 0; round < cfg.max_rounds; ++round)
    {
        ...
        {
            int cur_hop = Abc_NtkComputeHopNum(pNtk);
            printf("csr2: phase0 round %2d  moves=%3d  cut-edges=%d  hop=%d/%d\n",
                                ...
                                int trajectory_id)
{
    ...
    if (cfg.verbose)
        printf("csr2: trajectory %d initial cut-edges = %d\n", trajectory_id, initial_cutedges);
    ...
    printf("csr2: cut-edges %d -> %d (after phase0=%d, after phase1=%d, after phase2=%d)\n",
           initial_cutedges, final_cutedges, after_phase0, after_phase1, after_phase2);
    printf("csr2: phase0 %d moves / %d swaps / %d compound; phase1 %d attempts / %d successes; phase2 %d replications\n",
           total_moves, total_swaps, total_compound, total_attempts,
           total_successes, total_replications);
    if (cfg.verbose)
        printf("csr2: phase1 plans single-remove=%d joint-remove=%d joint-replace=%d multi-divisor=%d\n",
...
bool ApplyCsr(Abc_Frame_t *pAbc, const Config &cfg)
{
    ...
    if (!pEntry)
    {
        printf("csr2: current network is empty\n");
        return false;
    }
    if (!Abc_NtkIsLogic(pEntry))
    {
        printf("csr2: network must be logic (not AIG)\n");
        return false;
    }
    if (!pEntry->pPdb)
    {
        printf("csr2: no partition database (run hpart first)\n");
        return false;
    }
    ...
    for (int trajectory_id = 0; trajectory_id < cfg.num_trajectories; ++trajectory_id)
    {
        ...
        printf("csr2: trajectory %d cut-edge=%d cut-net=%d hop=%d nodes=%d sec=%.2f valid=%d\n",
    ...
    if (!selected.pNtk)
        return false;
    printf("csr2: selected trajectory %d\n", selected.trajectory_id);
