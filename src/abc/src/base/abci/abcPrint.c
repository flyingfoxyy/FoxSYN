...
extern "C" {
#endif
extern int Fox_TimerTryMaxArrival( Abc_Ntk_t * pNtk, float * pMaxArrival );
#ifdef __cplusplus
}
#endif
...
//        Abc_Print( 1,"  var = %5d", Abc_NtkCiNum(pNtk) + Abc_NtkCoNum(pNtk)+Abc_NtkGetMultiRefNum(pNtk) );
    ...
    {
        int CutSizeNow = Abc_NtkComputeCutSize( pNtk );
        int HopNumNow = Abc_NtkComputeHopNum( pNtk );
        if ( CutSizeNow >= 0 )
            CutSize = CutSizeNow;
        if ( HopNumNow >= 0 )
            HopNum = HopNumNow;
        ...
        if ( pNtk->pPdb )
            fHasMaxArrival = Fox_TimerTryMaxArrival( pNtk, &MaxArrival );
